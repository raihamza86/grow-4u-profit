const PriceSection = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 w-full max-w-6xl mx-auto gap-4 py-6">
      {total.map((item, index) => (
        <div
          key={index}
          className="bg-[radial-gradient(circle,#FFC107,_#6dd5ed)] rounded-xl p-4 flex flex-col"
        >
          <p className="text-white md:text-lg font-semibold">{item.heading}</p>
          <p className="text-white md:text-xl font-bold">{item.price}</p>
        </div>
      ))}
    </div>
  );
